...
import { calculateGoalProgress } from "@/lib/utils/goal-progress";
import { auth } from "@/lib/auth";
import { isSameDay, startOfDay, subDays } from "date-fns";
import { revalidatePath } from "next/cache";
import { GoalCreateInput } from "@/lib/schemas/goal";
...
export async function calculateGoalStreak(userId: string) {
  try {
    ...
    console.log(`Recent sessions for user ${userId}`, recentSessions);
...
export async function getGoals(subjectId?: string, topicId?: string) {
  try {
    ...
    const goalsWithProgress = await Promise.all(
      goals.map(async (goal) => {
        const progress = await calculateGoalProgress(goal);
        const status: GoalStatus = progress >= goal.target ? "COMPLETED" : progress > 0 ? "IN_PROGRESS" : "NOT_STARTED";
    ...
    return { goals: goalsWithProgress, summary };
  } catch (error) {
    console.error("Error getting goals", error);
    return { error: "Error getting goals" };
  }
}
  ...
  try {
    ...
    const goal = await prisma.goal.create({
      data: {
        ...
        target: input.target,
        deadline: input.deadline || new Date().setFullYear(new Date().getFullYear() + 10).toLocaleString(),
